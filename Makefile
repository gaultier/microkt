...
CFLAGS_COMMON =-Wall -Wextra -pedantic -g -std=c99 -march=native -fno-omit-frame-pointer -fstrict-aliasing -fPIC -pie -DWITH_LOGS=$(WITH_LOGS) -DWITH_ASAN=$(WITH_ASAN) $(CFLAGS_ASAN_$(WITH_ASAN)) -DLD='"$(CC)"' -DAS='"$(AS)"' -D_POSIX_C_SOURCE=200112L
