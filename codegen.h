...
//                                              emit_op_id_t op) {
//    PG_ASSERT_COND((void*)emitter, !=, NULL, "%p");
//    emit_op_t* const current = emit_op_get(emitter,
//    emitter->em_current_block);
//
...
//    PG_ASSERT_COND(
//        node->node_kind == NODE_STRING || node->node_kind ==
//        NODE_KEYWORD_BOOL,
//        ==, 1, "%d");
//
...
//        emitter, OP(emitter, OP_CALL("print_int", sizeof("print_int"),
//        args)));
//}
//
...
//                                   OP(emitter,
//                                   OP_REGISTER(emit_fn_arg(0))))));
//
//    buf_push(call_args,
//             OP(emitter, OP_ASSIGN(OP(emitter, OP_I64((int64_t)string_len)),
//                                   OP(emitter,
//                                   OP_REGISTER(emit_fn_arg(1))))));
//
//    emit_add_to_current_block(
//        emitter, OP(emitter, OP_CALL("print", sizeof("print"), call_args)));
//}
//
static void emit(emit_t* emitter, const parser_t* parser, FILE* asm_file) {
    PG_ASSERT_COND((void*)parser, !=, NULL, "%p");
...
//                println(".global %.*s", (int)block.cb_name_len,
//                block.cb_name);
//
...
//            println("%.*s+%d(%s) ", (int)p.pt_name_len, p.pt_name,
//            p.pt_offset,
...
//    //      const emit_op_string_label_t s = AS_STRING_LABEL(*op);
//    //      println(".L%d: .ascii \"%.*s\"", s.sl_label_id,
//    //      (int)s.sl_string_len,
//    //              s.sl_string);
//    //  }
//
